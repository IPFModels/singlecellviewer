...
server <- function(input, output,session) {
  ...
  dataModal <- function(failed = FALSE) {
    modalDialog(
      h3("Unpublished data. Please re-authenticate for security"),
  ...
  datasetTable <- reactive({
    ...
    colnames(file)=c("Project Name","Project Description","Organism","File type")
  ...
  prjsumm <- reactive({
    ...
    rownames(df)=c("Project name","Project Description","Organism","Total number of cells before filtration","Total nummber of cells after filtering","Total number of genes","Dimension","Number of mitochondrial genes filtered","","Cluster-wise number of cells",as.character(c.cnt$Var1))
    colnames(df)<- NULL
    return(df)
  })
  ...
  comptsne2 = reactive({
    ...
    p2 <- add_sub(p, paste(projectname(),"_CompareDR",sep=""), x = 0.87,vpadding = grid::unit(1, "lines"),size=11)
    ggdraw(p2)
    ggdraw(p2)
  })
  ...
  output$downloadtsneplot <- downloadHandler(
    filename = function() {
      paste0(projectname(),"_CompareDR.pdf",sep="")
    },
  ...
  heatmap <- reactive({
    withProgress(session = session, message = 'Generating...',detail = 'Please Wait...',{
      ...
      if(input$shmptype =="deggene"){
        ...
        markers=scrna@misc$findallmarkers
        markers= markers %>% group_by(cluster) %>% top_n(input$topn, avg_log2FC)
  ...
  output$bigene_rangea2 <- renderUI({
    withProgress(session = session, message = 'Generating...',detail = 'Please Wait...',{
      ...
      sliderInput("bigene_rangea2", "Expression Limit of Ligand Gene (log2 UMI)",
                  min = 0, max = r[2], value = c(r[1],r[2]),step=.25)
    })
  })
  ...
  output$bigene_rangeb2 <- renderUI({
    withProgress(session = session, message = 'Generating...',detail = 'Please Wait...',{
      ...
      sliderInput("bigene_rangeb2", "Expression Limit of Receptor Gene (log2 UMI)",
                  min = 0, max = r[2], value = c(r[1],r[2]),step=.25)
    })
  })
  ...
  output$source <- renderUI({
    ...
    # if(org=="mouse"){
    #   data(Mm_PairsLigRec)
    #   rl=mm}else if(org=="human"){
    #     data(Hs_PairsLigRec)
    #     rl=hs}
    options=c("novel","known")
  ...
  bigeneplot2 <- reactive({
    withProgress(session = session, message = 'Generating...',detail = 'Please Wait...',{
      ...
      p=bigene_plot(fileload(),
                    ...
                    limita=input$bigene_rangea2,
                    limitb=input$bigene_rangeb2,
